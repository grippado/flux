...
**Com `--save <dir>`:** gravar o parecer em `<dir>/YYYY-MM-DD-flux-peek-{alvo-slug}.md` além de imprimir no chat. O arquivo inclui um frontmatter com bloco `provenance` seguindo a disciplina de `${FLUX_ROOT}/shared/review-artifact-template.md` (seção "Bloco `provenance`" do `board-template.md`), com `generator: "flux-peek"` e `stamp: "<harness> | flux:peek@<flux_version>"`. Não persiste no vault automaticamente (escolha explícita do usuário).
