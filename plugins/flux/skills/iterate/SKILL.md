...
carimbo: {harness} | flux:iterate@{flux_version}
```
````
...
6. **Salvar** o output do `<ANSWERER>` no arquivo calculado (Write tool), com frontmatter mínimo de roteamento (`type: pr-review`, `context: <VAULT_CTX>`, `repo: <repo-slug>`, `pr:`, `date:`, `pending_organize: true`) — sem esse sinal o `/organize` não sabe para qual contexto promover — e com o bloco `provenance` (`generator: "flux-iterate"`, mesmo `session_sources` do `board-template.md`, ver lá, seção "Bloco `provenance`"): é nota escrita por uma sessão como qualquer outra, e ficar de fora quebraria a filtrabilidade que o campo existe para dar. **Nunca** escrever no GitHub.
7. Anunciar no chat:
   ```
